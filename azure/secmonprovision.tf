resource "null_resource" "secmonProvision" {
  ...
  provisioner "remote-exec" {
	...
	"echo elasticsearch.password: \"changeme\" | sudo tee -a /etc/kibana/kibana.yml",
        "sudo systemctl start kibana",
	"sudo sed -i.bak '38,56d;89,90d' /etc/nginx/nginx.conf",
        "sudo htpasswd -cb /etc/nginx/htpasswd.users ${var.stackmonwebuser} ${var.stackmonwebpass}",
  ...
  provisioner "remote-exec" {
	 ...
	 "cd beats-dashboards-*",
         "./load.sh -user kibana:changeme",
         "echo 'curl -u kibana:changeme -XPUT 'http://localhost:9200/_template/filebeat?pretty' -d@../filebeat-index-template.json'",
